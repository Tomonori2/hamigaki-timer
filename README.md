...
`index.html` の `FEEDBACK_URL` にURLを入れると、「⚙️ めいぼ」画面に「✉️ ごいけんを おくる」ボタンが表示されます。空のままなら何も表示されません。

```js
// Googleフォームの場合
const FEEDBACK_URL = 'https://forms.gle/xxxxxxxx';
// メールの場合（件名と、なまえを含まない利用状況が下書きに入ります）
const FEEDBACK_URL = 'mailto:you@example.com';
```

送られる内容に園児の名前やクラス名は含まれません（人数・記録日数・タイマーの長さのみ）。
